...
namespace MCEBuddy.MetaData
{
    public static class TMDB
    {
        ...
        /// </summary>
        private static bool MatchByBroadcastTime(TMDbClient client, VideoTags videoTags, TvShow tvShow)
        {
                ...
                for (int eNo = 0; eNo <= season.Episodes.Count; eNo++)
                {
                    ...
                    if (!String.IsNullOrWhiteSpace(episodeName))
                    {
                            ...
                            (firstAired.Date == videoTags.OriginalBroadcastDateTime.ToUniversalTime().Date))
                        {
                            ...
                            if (!String.IsNullOrWhiteSpace(seriesName) && String.IsNullOrWhiteSpace(videoTags.Title)) videoTags.Title = seriesName;
