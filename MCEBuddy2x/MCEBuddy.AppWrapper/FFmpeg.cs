        ...
        protected override void OutputHandler(object sendingProcess, System.Diagnostics.DataReceivedEventArgs ConsoleOutput)
        {
            try
            {
                ...
                if (!String.IsNullOrEmpty(ConsoleOutput.Data))
                {
                    ...
                    if (StdOut.Contains(@"av_interleaved_write_frame(): Invalid argument") ||
                        StdOut.Contains(@"av_interleaved_write_frame(): Invalid data found when processing input"))
                        _encodeError = true;

                    if (StdOut.Contains(@"use audio bitstream filter 'aac_adtstoasc' to fix it"))
                        _aacadtstoascError = true;

                    if (StdOut.Contains("use the h264_mp4toannexb bitstream filter"))
