    ...
    public class Core : ICore 
    {
        ...
        public List<EventLogEntry> GetWindowsEventLogs()
        {
            ...
            try
            {
                ...
                // Sort the entries by Time Generated
                EventLogEntry[] eventLogArray = new EventLogEntry[eventLog.Entries.Count];
                eventLog.Entries.CopyTo(eventLogArray, 0);
                List<EventLogEntry> logEntries = new List<EventLogEntry>(eventLogArray);
                IEnumerable<EventLogEntry> sortedEntries = from entity in logEntries.AsEnumerable() orderby entity.TimeGenerated descending select entity;

                foreach (EventLogEntry logEntry in sortedEntries)
                {
                    if (logEntry.Source == GlobalDefs.MCEBUDDY_EVENT_LOG_SOURCE)
                        retList.Add(logEntry);

                    if (retList.Count >= GlobalDefs.MAX_EVENT_MESSAGES_TRANSFER) // To prevent message buffer overflow while communicating
                        break;
                }
            }
            catch (Exception e)
