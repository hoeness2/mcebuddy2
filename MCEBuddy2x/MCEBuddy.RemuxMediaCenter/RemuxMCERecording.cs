...
namespace MCEBuddy.RemuxMediaCenter
{
    public class RemuxMCERecording
    {
        ...
        private static string[] _mpeg1Codecs = { "mpegvideo", "mpeg1video" };
        private static string[] _mpeg2Codecs = { "mpeg2video" };
        private static string[] _mpeg4Codecs = { "msmpeg4", "msmpeg4v1", "msmpeg4v2", "mpeg4", "mpeg4video", "h263", "h263i", "h263p", "h264", "h.264", "avc", "xvid", "divx" };
        private static string[] _supportedCodecs = _mpeg1Codecs.Union(_mpeg2Codecs).Union(_mpeg4Codecs).ToArray();
        ...
        public bool Remux()
        {
            ...
            if (Util.FilePaths.CleanExt(_RecordingFile) == ".tivo") // Check for TiVO files
            {
                ...
                if (GlobalDefs.IsEngineRunningAsService)
                    _jobLog.WriteEntry(this, "You need to start MCEBuddy engine as a Command line program. TiVO Desktop Directshow decryption filters do not work with a Windows Service.", Log.LogEntryType.Warning);
            ...
            else if (Util.FilePaths.CleanExt(_RecordingFile) == ".wtv") // WTV can contain both MPEG2 and MPEG4 video
            {
                ...
                {
                    if (_allowH264CopyRemuxing || _mpeg2Codecs.Any(s => s.Contains(_RecordingFileMediaInfo.MediaInfo.VideoInfo.VideoCodec.ToLower()))) // By default we only copy mpeg2 streams and transcode mpeg4 to mpeg2 unless we are asked to copy mpeg4
                    {
            ...
            // TODO: We need to find a better way to handle remuxsupp (rebuild the source?)
            ...
            {
                // TODO: Some H264 files with REmuxsupp just hang it! Remuxsupp is not good with H264, unless asked for now limit RemuxSupp to mpeg2video files
                if (_mpeg2Codecs.Any(s => s.Contains(_RecordingFileMediaInfo.MediaInfo.VideoInfo.VideoCodec.ToLower()))) // By default we only copy mpeg2 streams and transcode mpeg4 to mpeg2 unless we are asked to copy mpeg4
                {
                    // Otherwise try a direct byte mux
                    _jobStatus.CurrentAction = Localise.GetPhrase("Byte stream remuxing");
                    if (RemuxSuppWTV())
