...
namespace MCEBuddy.RemuxMediaCenter
{
    public class ExtractWithGraph : IDisposable
    {
            ...
            while (FilterPins.Next(pins.Length, pins, fetched) == 0)
            {
                ...
                while (enummtypes.Next(1, ptypes, fetched2) == 0)
                {
                        ...
                        (mtypes.majorType == MediaType.Mpeg2Sections && mtypes.subType == MediaSubType.None && mtypes.formatType == FormatType.None))
                    {
                        ...
                        else if ((mtypes.majorType == MediaType.Audio) && ((_extractMediaType & ExtractMediaType.Audio) != 0)) // Audio
                        {
        ...
        public void RunGraph()
        {
            ...
            long sourceSize = Util.FileIO.FileSize(_SourceFile); // Sanity checking
            if (sourceSize <= 0)
            {
            ...
            while (!stop)
            {
                ...
                if (sourceSize > 0)
                {
                    ...
                    if ((_extractMediaType & ExtractMediaType.Video) != 0)
                    {
                        ...
                        {
                            _jobLog.WriteEntry(this, "Extracted video stream is greater than " + INFINITE_LOOP_CHECK_THRESHOLD.ToString(System.Globalization.CultureInfo.InvariantCulture) + " times the source file size " + (sourceSize/ 1024).ToString("N", System.Globalization.CultureInfo.InvariantCulture) + " [KB].\r\nExtraction likely hung, terminating streams extraction.", Log.LogEntryType.Error);
                            stop = true;
                            AbortError = true;
                            mediaControl.Stop();
                            break;
                    ...
                    if ((_extractMediaType & ExtractMediaType.Audio) != 0)
                    {
                        foreach (string audioPart in _AudioParts)
                        {
                            ...
                            {
                                _jobLog.WriteEntry(this, "Extracted audio stream is greater than " + INFINITE_LOOP_CHECK_THRESHOLD.ToString(System.Globalization.CultureInfo.InvariantCulture) + " times the source file size " + (sourceSize / 1024).ToString("N", System.Globalization.CultureInfo.InvariantCulture) + " [KB].\r\nExtraction likely hung, terminating streams extraction.", Log.LogEntryType.Error);
                                stop = true;
                                AbortError = true;
                                mediaControl.Stop();
                                break;
                    ...
                    if ((_extractMediaType & ExtractMediaType.Subtitle) != 0)
                    {
                        foreach (string subtitlePart in _SubtitleParts)
                        {
                            ...
                            {
                                _jobLog.WriteEntry(this, "Extracted subtitle stream is greater than " + INFINITE_LOOP_CHECK_THRESHOLD.ToString(System.Globalization.CultureInfo.InvariantCulture) + " times the source file size " + (sourceSize / 1024).ToString("N", System.Globalization.CultureInfo.InvariantCulture) + " [KB].\r\nExtraction likely hung, terminating streams extraction.", Log.LogEntryType.Error);
                                stop = true;
                                AbortError = true;
                                mediaControl.Stop();
                                break;
                    ...
                    _jobLog.WriteEntry(this, "Percentage complete : " + _jobStatus.PercentageComplete.ToString("0.00", System.Globalization.CultureInfo.InvariantCulture) + " %", Log.LogEntryType.Debug); // Write to file
                }
